	...
	
{
	...
	if (OnlineSubsystem)
	{
		OnlineSessionsInterface = OnlineSubsystem->GetSessionInterface();
...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	if(!OnlineSessionsInterface.IsValid())
	{
		return;
	}
	auto ExistingSession = OnlineSessionsInterface->GetNamedSession(NAME_GameSession);
	if(ExistingSession != nullptr)
	{
		OnlineSessionsInterface->DestroySession(NAME_GameSession);
	}
	OnlineSessionsInterface->AddOnCreateSessionCompleteDelegate_Handle(CreateSessionCompleteDelegate);
	...
	OnlineSessionsInterface->CreateSession(*LocalPlayer->GetPreferredUniqueNetId(), NAME_GameSession, *SessionSettings);
}	

void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccess)
{
	if (bWasSuccess)
	{
		if (GEngine)
